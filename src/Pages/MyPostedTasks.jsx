...
const MyPostedTasks = () => {
 ...
 const navigate = useNavigate();
  useEffect(() => {
    ...
    fetch(`http://localhost:5000/my-tasks?email=${user.email}`)
      .then((res) => res.json())
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#b10ebd] via-[#8300a4] to-[#ff2e9a] p-4">
      <div className="max-w-6xl mx-auto">
        <h2 className="text-4xl font-bold text-center text-white mb-8 drop-shadow-lg">🎯 My Posted Tasks</h2>
        ...
        <div className="overflow-x-auto rounded-xl shadow-2xl border border-pink-300">
          <table className="min-w-full divide-y divide-pink-200 bg-white rounded-xl overflow-hidden">
            ...
            <tbody className="bg-white divide-y divide-gray-200 text-sm">
              {tasks.length > 0 ? (
                tasks.map((task) => (
                  <tr key={task._id} className="hover:bg-pink-50 transition duration-150">
                    <td className="px-3 py-4 font-medium text-gray-800">{task.title}</td>
                    <td className="px-3 py-4 text-gray-600">{new Date(task.deadline).toLocaleDateString()}</td>
